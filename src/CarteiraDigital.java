...
public class CarteiraDigital {
    ...
    public void adicionarSaldo(double valor) {
        if (isValorPositivo(valor)) {
            ...
            exibirMensagem("Valor inválido para adicionar. O valor deve ser positivo.");
        }
    }
    ...
    public void realizarPagamento(double valor) {
        if (isPagamentoValido(valor)) {
            saldo -= valor;
            registrarHistorico("Pagamento de " + valor + " realizado.");
            mostrarSaldoAtual();
        } else {
            exibirMensagem("Erro: Saldo insuficiente. Seu saldo atual é R$" + saldoFormatado());
        }
    }
